...
"""
On rancheros image, provision should be disabled.
"""
class RancherOSProvisionHandler(ProvisionHandler):
    def __init__(self):
        super(RancherOSProvisionHandler, self).__init__()

    def run(self):
        if conf.get_provision_enabled():
            logger.error("Provision.Enabled=y not supported")
            return

        logger.info("run RancherOS provision handler")
        ...
        try:
            ...
            logger.error("Failed save CustomData: {0}".format(e))
            self.report_not_ready("save_customdata failed", ustr(e))
